...
const getAllDiscount = async (req, res) => {
    try {
        ...
        if(result.length === 0){
            return res.status(200).json({statusCode: 200, msg: 'Discount is empty!!1', })
        }
...
const createDiscountEmail = async (req, res) => {
    ...
    try {
        if(!data?.email){
            return res.status(200).json({statusCode: 200, msg: 'Email is not empty!'})
        }
        const discountCheck = await DiscountSchema.find({email: email});
        if(discountCheck.length !== 0) {
            return res.status(400).json({statusCode: 400, msg: 'Email already used!!!'})
        }
        const discountId = `NEW_MEMBER${new Date().valueOf()}`;

        await sendMail({template: "discount", email: email, subject: 'Discount for you <3!!!!', templateVars: {discountId: discountId, urlWeb: process.env.URL_CLIENT} });
...
const getDiscountWithCode = async (req, res) => {
    ...
    try {
        ...
        if(discount.length === 0){
            return res.status(400).json({statusCode: 400, msg: 'Discount is not existed!'})
        }
        return res.status(200).json({statusCode: 200, msg: 'Success!', data: discount[0]})
    } catch (error) {
        return res.status(500).json({error})
    }
}
